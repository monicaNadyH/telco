    ...
    toneAnalyzer.tone(
      ...
      function(err, tone) {
        ...
        //  console.log('Emotion ====== ', tone.document_tone.tone_categories[0].tones);
          ...
          for (let i = 0; i < len; i++) {
var percentage = emotionTones[i].score * 100;
              payload.context['emotion_score'] = percentage ;
      ...
      //  tone_anger_score = toneAngerScore;
        ...
        if (payload.input.text != '') {
          ...
          nlu.analyze(parameters, function(err, response) {
            ...
            //  payload.context['nlu_output'] = nluOutput.keywords[0].emotion;

            //  console.log('NLU ========= ', nluOutput.keywords[0].emotion, ' =======end');


              // console.log('NLU ========= ', nluOutput, ' =======end');
              // identify location
              ...
              //  console.log('entries ========= ', JSON.parse(entities), ' =======end');
            ...
            /*  const entities = nluOutput.entities;
              let sentiment = entities.map(function(entry) {
                if (entry.type == 'sentiment') {
                  return entry.text;
                }
              });
              sentiment = sentiment.filter(function(entry) {
                if (entry != null) {
                  return entry;
                }
              });
              if (sentiment.length > 0) {
                payload.context['sentiment'] = sentiment[0];
                console.log('sentiment = ', payload.context['sentiment']);
              } else {
                payload.context['sentiment'] = '';
              }*/
          ...
          /*  const entities = nluOutput.entities;
              let getemotion = entities.map(function(entry) {
                if (entry.type == 'Emotion') {
                  return entry.text;
                }
              });
              getemotion = getemotion.filter(function(entry) {
                if (entry != null) {
                  return entry;
                }
              });
              if (getemotion.length > 0) {
                payload.context['Emotion'] = getemotion[0];
                console.log('Emotion = ', payload.context['Emotion']);
              } else {
  ...
  //   // secure)
   nano.db.get('telco', function(err) {
     if (err) {
       console.error(err);
       nano.db.create('telco', function(errCreate) {
         console.error(errCreate);
         logs = nano.db.use('telco');
   ...
   app.post('/clearDb', auth, function(req, res) {
     nano.db.destroy('telco', function() {
       nano.db.create('telco', function() {
         logs = nano.db.use('telco');
       });
     });
